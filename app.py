...
from scipy.ndimage import gaussian_filter1d, grey_erosion, grey_dilation, uniform_filter1d
...
class AppState:
    ...
    # NEW: Baseline correction fields
...
class BaselineCorrector:
    """Advanced baseline corrector with support for pybaselines methods"""
    ...
    def correct(self, x, y):
        """Apply baseline correction with selected method"""
        if self.method == 'none':
        ...
        try:
            ...
            methods = {
                ...
                'imor': (baseline.imor, {'half_window': half_window, 'tol': 1e-3, 'max_iter': 200}),
                'rolling_ball': (baseline.rolling_ball, {'half_window': half_window}),
                'rubberband': (baseline.rubberband, {'segments': 1, 'lam': lam}),
                'dietrich': (baseline.dietrich, {'poly_order': 5, 'num_std': 3.0}),
                'golotvin': (baseline.golotvin, {'half_window': half_window, 'num_std': 2.0}),
                'pspline_asls': (baseline.pspline_asls, {'lam': lam, 'p': p, 'num_knots': 100}),
...
class GaussianDeconvolver:
    ...
    def __init__(self, x_linear, y_original, use_log_x=True, use_log_y=False,
                 ...
                 smoothing_level='none', baseline_params=None):
        
        self.x_original = np.array(x_linear).copy()
        self.y_original_raw = np.array(y_original).copy()
        
        self.baseline_curve = np.zeros_like(y_original)
        self.baseline_info = {}
        ...
        if baseline_method != 'none' and baseline_params:
            ...
            y_corrected, baseline, info = corrector.correct(x_linear, y_original)
...
# ==================== STEP 2.5: BASELINE CORRECTION ====================
    ...
    with col1:
        ...
        if method_key in ['asls', 'airpls', 'arpls', 'drpls', 'iarpls', 'psalsa', 'derpsalsa', 'pspline_asls']:
            lam = st.number_input(
                "Smoothing (λ)",
                min_value=1e0,
                max_value=1e9,
            ...
            half_window = st.slider(
                ...
                value=min(st.session_state.app_state.baseline_half_window, len(x) // 4),
        ...
        elif method_key in ['dietrich']:
            poly_order = st.slider(
                "Polynomial order",
                min_value=1,
                max_value=10,
                value=st.session_state.app_state.baseline_poly_order,
            ...
            num_std = st.slider(
                ...
                min_value=1.0,
                max_value=5.0,
                value=float(st.session_state.app_state.baseline_num_std),
                step=0.1,
        ...
        with col_apply:
            if st.button("✅ Apply Correction", type="primary", use_container_width=True):
                with st.spinner("Applying baseline correction..."):
                    ...
                    st.session_state.app_state.baseline_method = method_key
    ...
    with col2:
        ...
        if st.session_state.app_state.get('preview_baseline', False):
            ...
            if hasattr(st.session_state.app_state, 'baseline_curve') and st.session_state.app_state.baseline_curve is not None:
                ...
                if hasattr(st.session_state.app_state.baseline_corrector, 'get_quality_metrics'):
...
# ==================== STEP 3: PEAK DETECTION ====================
    ...
    if st.session_state.app_state.deconvolver is None:
        ...
        baseline_params = st.session_state.app_state.baseline_params if st.session_state.app_state.baseline_method != 'none' else None
        ...
        if (st.session_state.app_state.peak_info is not None and 
            ...
            with tab3:
                ...
                edited_df = st.data_editor(
                    ...
                    hide_index=True,
                    disabled=["ID", "Source", "X Center", "Y Amplitude", "Estimated Sigma"],
                )
